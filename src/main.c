...
static inline void pre_arithmetic_reg(CodeBuffer *cb) {
	cb_emit_u8(cb, 0x48);
	cb_emit_u8(cb, 0x01);
}
...
static inline void emit_sub_reg(CodeBuffer *cb, int32_t dst, int32_t src) {
	cb_emit_u8(cb, 0x48);
	cb_emit_u8(cb, 0x29);
	cb_emit_u8(cb, 0xC0 | (src << 3) | dst);
}

static inline void emit_mul_reg(CodeBuffer *cb, int32_t dst, int32_t src) {
	if (dst != 0) {
		cb_emit_u8(cb, 0x48);
		cb_emit_u8(cb, 0x89);
	...
	cb_emit_u8(cb, 0x48);
	cb_emit_u8(cb, 0xF7);
	cb_emit_u8(cb, 0xE0 | src);

	if (dst != 0) {
		cb_emit_u8(cb, 0x48);
		cb_emit_u8(cb, 0x89);
		cb_emit_u8(cb, 0xC0 | (0 << 3) | dst);
	}
}

static inline void emit_div_reg(CodeBuffer *cb, int32_t dst, int32_t src) {
	if (dst != 0) {
		cb_emit_u8(cb, 0x48);
		cb_emit_u8(cb, 0x89);
		cb_emit_u8(cb, 0xC0 | (dst << 3) | 0);
